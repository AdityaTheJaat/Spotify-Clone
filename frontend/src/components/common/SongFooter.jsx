        ...
        token ? (
            <div className='w-full flex justify-between items-center h-full '>
              <div className='w-1/4 flex items-center'>
                <img className='h-14 w-14 rounded' src={currentSong.thumbnail} alt="" />
                <div className='pl-4'>
                  <p className='text-sm'>{currentSong.name}</p>
                  {
                    currentSong.artist ? (<p className='text-xs text-gray-500'>{currentSong.artist.firstName + " " + currentSong.artist.lastName}</p>) : (<p className='text-xs text-gray-500'>{currentSong.firstName + " " + currentSong.lastName}</p>)
                  }
                </div>
              </div>
              ...
              <div className='w-1/4 flex justify-end gap-3 h-full'>
                ...
                {/* <button onClick={clickHandler} className='flex-col'>
                  <img src={bheart} alt="" className='w-8 h-8 mx-auto' />
          ...
          (
            <div className="w-full flex justify-between items-center h-1/10 p-2">
              ...
              <div>
                <Link to='/signup' className='bg-white text-black rounded-3xl px-7 py-2'>Sign up for free</Link>
              </div>
            </div>
          )
      }
